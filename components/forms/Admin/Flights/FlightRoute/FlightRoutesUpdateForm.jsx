...
import {
  ...
  useUpdateFlightRoute,
} from "@/hooks/Admin/useFlightRoute";
import { flightRouteUpdateValidation } from "@/lib/validations/Admin/flightRouteCreateValidation";
...
function FlightRouteUpdateForm({ RouteId }) {
  ...
  const {
    ...
    resolver: zodResolver(flightRouteUpdateValidation),
    defaultValues: {
      from_airport_id: "",
      to_airport_id: "",
      distance_km: "",
  ...
  useEffect(() => {
    if (route) {
      reset({
        from_airport_id: String(route.from_airport_id ?? ""),
        to_airport_id: String(route.to_airport_id ?? ""),
  ...
  return (
    <div className="bg-gradient-to-br from-gray-50 via-white to-blue-50 min-h-screen p-4 md:p-8">
      ...
      <div className="mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Flight Route</h1>
          <p className="text-gray-500 mt-1">Update flight route information</p>
        </div>
      </div>
...
export default FlightRouteUpdateForm;