...
//  Validation Schema 
const flightClassUpdateValidation = z.object({
...
//  Main Form 
function FlightClassUpdateForm({ id }) {
  const router = useRouter();

  const { data: flightData, isLoading: flightLoading } = useAllFlight();
  const { data: classData, isLoading: classLoading } = useFlightClassById({
    id,
  });

  const { mutate, isPending } = useUpdateFlightClass();
  ...
  const {
    ...
    resolver: zodResolver(flightClassUpdateValidation),
  });
  ...
  useEffect(() => {
    ...
    const d = classData?.data;
    const fr = d.fare_rule;

    reset({
      // ── FlightClass fields ──
      flight_id: String(d.flight_id ?? ""),
      class_name: d.class_name ?? "",
      fare_code: d.fare_code ?? "",
      base_fare: String(d.base_fare ?? ""),
      currency: d.currency ?? "",
      total_seats: String(d.total_seats ?? ""),
      seats_available: String(d.seats_available ?? ""),
      seats_booked: String(d.seats_booked ?? ""),
      is_active: d.is_active == 1 ? "true" : "false",

      // ── FareRule fields ──
      is_refundable: fr?.is_refundable == 1 ? "true" : "false",
      is_partially_refundable:
        fr?.is_partially_refundable == 1 ? "true" : "false",
      cancellation_penalty: String(fr?.cancellation_penalty ?? ""),
      change_penalty: String(fr?.change_penalty ?? ""),
      cabin_baggage_kg: String(fr?.cabin_baggage_kg ?? ""),
      checkin_baggage_kg: String(fr?.checkin_baggage_kg ?? ""),
      extra_baggage_fee_per_kg: String(fr?.extra_baggage_fee_per_kg ?? ""),
      notes: fr?.notes ?? "",
    });
  }, [classData, reset]);
  ...
  const onSubmit = async (data) => {
    try {
      // ── Guard: ensure id is valid before submitting ──
      if (!id) {
        return;
      }
      ...
      const payload = {
        ...
        fare_rule: {
          is_refundable: data.is_refundable === "true" ? 1 : 0,
          is_partially_refundable:
            data.is_partially_refundable === "true" ? 1 : 0,
      ...
      <div className="mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Edit seating class and fare rule details — ID #{id}
          </p>
        </div>
      </div>
            ...
            <div className="p-6">
              <div className="flex flex-col sm:flex-row justify-end gap-4">
                ...
                <Button
                  ...
                  disabled={isPending}
                >
                  {isPending ? "Updating..." : "Update Flight Class"}
                </Button>
              </div>
            </div>
          </div>
...
export default FlightClassUpdateForm;
